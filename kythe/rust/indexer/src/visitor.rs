...
use syntax::ast;
use syntax::source_map::{Pos, SourceMap, Span, Spanned};

/// Indexes all kythe entries except files
pub struct KytheVisitor<'a, 'tcx> {
    pub writer: &'a Box<dyn EntryWriter>,
...
impl<'a, 'tcx> KytheVisitor<'a, 'tcx> {
    ...
    fn anchor_from_span(&self, span: Span) -> VName {
        ...
        let path = start.sf.unmapped_path.as_ref().unwrap_or(&start.sf.name);
        self.anchor(&path.to_string(), start_byte, end_byte)
    }
    ...
    fn anchor_from_sub_span(&self, span: Span, sub: &str) -> Result<VName, String> {
        ...
        let sub_start = match snippet.find(sub) {
            None => {
                return Err(format!(
                    "Substring: '{}' not found in snippet '{}'",
                    sub, snippet
                ))
            }
            Some(s) => s,
        ...
        let path = start.sf.unmapped_path.as_ref().unwrap_or(&start.sf.name);
        Ok(self.anchor(&path.to_string(), start_byte, end_byte))
    }
    ...
    fn anchor(&self, file_name: &str, start_byte: usize, end_byte: usize) -> VName {
        ...
        let local_path = {
            ...
            file_path
                .strip_prefix(&self.crate_root_dir)
                .unwrap_or(file_path)
        };
        let vname = VName {
    ...
    fn vname_from_defid(&self, def_id: DefId) -> VName {
        ...
        let var_name = self.tcx.def_path_str(def_id);
    ...
    fn function_call(&self, call_node_id: ast::NodeId, callee_def_id: DefId) {
        let call_span = self.tcx.hir().span(call_node_id);
        ...
        self.writer
            .edge(&call_anchor_vname, EdgeKind::RefCall, &callee_vname);
        if let Some(ref parent_vname) = self.parent_vname.last() {
            self.writer
                .edge(&call_anchor_vname, EdgeKind::ChildOf, &parent_vname);
        }
    }
    ...
    fn opt_variant_did(&self, res: &Res) -> Option<DefId> {
        use rustc::hir::def::DefKind::*;
        match *res {
            Res::Def(def_kind, def_id) => match def_kind {
                Struct | Enum => Some(self.tcx.adt_def(def_id).did),
                Variant => Some(def_id),
                _ => None,
            },
        }
    ...
    pub fn index_files(&self) {
        ...
        for ref f in self.codemap.files().iter() {
            ...
            if let Some(ref content) = f.src {
                if let Some(ref abs_path) = f.unmapped_path {
                    let vname = self.file_vname(&abs_path.to_string());
...
fn is_from_macro(span: &Span) -> bool {
    span.source_callee().into() != u32::max_value()
}

impl<'v, 'tcx> Visitor<'v> for KytheVisitor<'v, 'tcx> {
    /// Enables recursing through nested items
    fn visit_nested_item(&mut self, id: ItemId) {
        let item = self.tcx.hir().expect_item(id.id);
    ...
    fn visit_pat(&mut self, pat: &'v Pat) {
        ...
        match pat.node {
            Binding(..) => {
                if let Some(def) = self.tcx.hir().opt_local_def_id(pat.hir_id) {
                    let local_vname = self.vname_from_defid(def);
                    let anchor_vname = self.anchor_from_span(pat.span);
                    self.writer
                        .edge(&anchor_vname, EdgeKind::DefinesBinding, &local_vname);
                    self.writer
                        .node(&local_vname, Fact::NodeKind, &NodeKind::Variable);
                }
            }

            Path(ref path) | Struct(ref path, _, _) | TupleStruct(ref path, _, _) => {
                let struct_def = self.tcx.hir().local_def_id(pat.hir_id);
    ...
    fn visit_block(&mut self, block: &'v Block) {
        ...
        use rustc::hir::{ExprKind, PatKind, Stmt, StmtKind};
        if let [Stmt { node: ref decl, .. }] = *block.stmts {
            if let StmtKind::Local(ref local) = decl {
                if let Some(ref expr) = local.init {
                    if let ExprKind::Match(ref base, ref outer_arms, ForLoopDesugar) = expr.node {
                        if let ExprKind::Call(_, ref args) = base.node {
                            if let ExprKind::Loop(ref block, _, _) = outer_arms[0].body.node {
                                if let Some(ref expr) = block.expr {
                                    if let ExprKind::Match(_, ref arms, _) = expr.node {
                                        if let PatKind::TupleStruct(_, ref pats, _) =
                                            arms[0].pats[0].node
                                        {
    ...
    fn visit_expr(&mut self, expr: &'v Expr) {
        ...
        match expr.node {
            ...
            ExprKind::Path(..) => {
                if let Some(def) = self.tcx.expect_def_or_none(expr.hir_id) {
    ...
    fn visit_fn(
        ...
        match kind {
            FnKind::ItemFn(n, _, _, _, _) | FnKind::Method(n, _, _, _) => {
                let fn_name = n.to_string();
                let def_id = self.tcx.hir().get_parent_did(body.hir_id);
                let fn_vname = self.vname_from_defid(def_id);
                let decl_vname = self.anchor_from_span(span);

                self.writer
                    .node(&fn_vname, Fact::NodeKind, &NodeKind::Function);
                self.writer
                    .node(&fn_vname, Fact::Complete, &Complete::Definition);
                self.writer.edge(&decl_vname, EdgeKind::Defines, &fn_vname);

                if let Ok(bind_vname) = self.anchor_from_sub_span(span, &fn_name) {
                    self.writer
                        .edge(&bind_vname, EdgeKind::DefinesBinding, &fn_vname)
                }
            }
            _ => (),
        };
    ...
    fn visit_item(&mut self, item: &'v hir::Item) {
        ...
        let def_name = item.ident.name.to_string();
        let def_vname = self.vname_from_defid(def_id);

        use rustc::hir::ItemKind::*;
        match item.node {
            Static(..) | Const(..) => {
                let kind = if let Static(..) = item.node {
                    NodeKind::Variable
                } else {
                    NodeKind::Constant
                };
                ...
                if let Ok(bind_vname) = self.anchor_from_sub_span(item.span, &def_name) {
                    self.writer
                        .edge(&bind_vname, EdgeKind::DefinesBinding, &def_vname)
                }
            }

            Struct(ref def, _) => {
                self.writer
                    .node(&def_vname, Fact::NodeKind, &NodeKind::Record);
                if let Ok(bind_vname) = self.anchor_from_sub_span(item.span, &def_name) {
                    self.writer
                        .edge(&bind_vname, EdgeKind::DefinesBinding, &def_vname);
                }
    ...
    fn visit_variant_data(
        ...
        let def_id = self.tcx.hir().local_def_id(v.ctor_hir_id());
        let def_vname = self.vname_from_defid(def_id);
        ...
        if let Ok(bind_vname) = self.anchor_from_sub_span(span, &name) {
            ...
            if let Some(ref parent_vname) = self.parent_vname.last() {
                self.writer
                    .edge(&def_vname, EdgeKind::ChildOf, parent_vname);
            }
        }
